    ...
    <div className="min-h-screen bg-[#020617] text-white flex font-sans overflow-hidden">
      <aside className="w-64 bg-[#0b0f1f] border-r border-white/10 flex-col hidden lg:flex relative z-20">
        <div className="p-6 border-b border-white/10 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-black text-base uppercase tracking-widest text-white">InternX CMS</h2>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-20 bg-[#0b0f1f]/80 backdrop-blur-md border-b border-white/10 flex items-center justify-between px-4 lg:px-8 shrink-0 relative z-10">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden text-slate-400 hover:text-white p-2 rounded-lg hover:bg-white/5">
          ...
          <div className="flex items-center gap-3 lg:gap-6 relative">
            <div className="relative">
              <button 
                ...
                className={`relative p-2 rounded-full transition-all ${isNotificationOpen ? 'bg-blue-500/20 text-blue-400' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
              >
                  ...
                  <div className="absolute right-0 mt-3 w-80 sm:w-96 bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl z-30 overflow-hidden animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="max-h-80 overflow-y-auto custom-scrollbar">
                      {notifications.length === 0 ? (
                        <div className="p-8 text-center text-slate-500 text-xs italic">No new alerts.</div>
                      ) : (
            ...
            <div className="relative">
              <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-3 p-1.5 pr-3 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 transition-all">
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs uppercase">
                  {session?.user?.name?.charAt(0) || 'A'}
                </div>
                <div className="hidden md:flex flex-col text-left">
                  <span className="text-xs font-bold text-white leading-none">{session?.user?.name}</span>
                  <span className="text-[10px] text-slate-400 mt-0.5 leading-none">{(session?.user as any)?.role || 'User'}</span>
                </div>
              ...
              {isProfileOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-48 bg-[#0f172a] border border-white/10 rounded-xl shadow-2xl z-30 py-2 animate-in fade-in slide-in-from-top-2">
                    <Link href="/dashboard/profile" onClick={() => setIsProfileOpen(false)} className="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors flex items-center gap-2">
                      <UserCircle className="w-4 h-4" /> My Profile
                    </Link>
                    <button onClick={() => signOut({ callbackUrl: '/login' })} className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-red-500/10 transition-colors flex items-center gap-2 mt-1 border-t border-white/5 pt-3">
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden relative bg-[#020617] p-4 lg:p-8 custom-scrollbar">
          {children}
        </main>
      </div>
    </div>
